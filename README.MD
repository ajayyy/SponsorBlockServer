...
This uses a Postgres database to hold all the timing data.
...
Ensure all the tests pass with `npm test`. To run tests, make sure the `test-database` from `docker/docker-compose.yml`.

Run the server with `npm start`. To run the server, make sure all the containers in `docker/docker-compose.yml` are running.
